...
namespace SysCommon
{
	...
	class IInterruptable
	{
		...
		public:
			 ...
			 */
			virtual WaitResult visit( NATIVE_INTERRUPT& threadInterrupt, DWORD timeout ) = 0;
	...
	class Thread : public IRunnable
	{
		...
		private:
			...
			static DWORD mainThreadSysID;
			static std::map<unsigned long, Thread*> threadMap;
			static std::map<DWORD, Thread*> nativeThreadMap;
		...
		public:
			 ...
			 * completed.
			 */
			WaitResult acceptInterruptable( IInterruptable* interruptable, DWORD timeout );
		...
		public:
			 ...
			 */
			static void sleep( DWORD millis ) throw ( InterruptedException );
			
		private:
			static String generateAnonymousThreadName();
			static DWORD WINAPI threadEntry( LPVOID threadInstance );
